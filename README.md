...
- **EPUB互換性修正**: 一部の電子書籍端末（xteink X4等のESP32ベースリーダー）でEPUBが開けない問題を修正
  - toc.ncx（EPUB2互換目次）を追加し、EPUB2/3両対応に
  - toc.xhtmlの章タイトルを`<span>`で囲みEPUB仕様に準拠
  - UUIDを正規フォーマットに変更
- **ダウンロード速度改善**: ウェイト方式をnarou.rb準拠のバッチウェイトに変更（適応型インターバルから変更）
  - http.client持続接続による高速化
  - YAMLシリアライズにCSafeDumperを使用（17倍高速）
- **削除機能改善**: 小説削除時にダウンロード済みデータも削除するように変更
- **変換品質改善**: converterのstashキーにPUA文字列を使用し、数字変換パイプラインとの干渉を完全に回避
- **GUI改善**: ログ表示で完了行をハイライト表示、更新なしの場合もEPUB変換を実行（設定変更への対応）
- **EPUBセクション順序修正**: セクションYAMLのソートが文字列順（1,10,11,...）になっていたのを数値順（1,2,3,...）に修正
